...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["pdfFile"]["name"])) {
    $file = $_FILES["pdfFile"];
    $title = $_POST["title"];
    $author = $_POST["author"];
	$major = $_POST["major"];
 ...
 // Get the current date
    ...
    $targetDir = "PDFs/"; // Specify the directory where you want to store the uploaded PDFs
    ...
    if (move_uploaded_file($file["tmp_name"], $targetFile)) {
        ...
        // Prepare and execute the SQL statement to insert the file details into the 'vid' table
			$sql = "INSERT INTO vid (file, title, author, major, education_level, description, username_upload, upload_date) VALUES ('$filePath', '$title', '$author', '$major', '$educationLevel', '$description', '$loggedUser', '$uploadDate')";        if ($conn->query($sql) === true) {
            echo "PDF uploaded and saved to the database.";

            // Redirect to pdfimg.php with the uploaded file path
            header("Location: pdfimg.php?file=" . urlencode($filePath));
            exit();
        } else {
            echo "Error inserting PDF into the database: " . $conn->error;
        }
    } else {
        echo "Error uploading PDF: " . $_FILES["pdfFile"]["error"];
    }
} else {
    // Display the HTML form for uploading a PDF
    ?>
