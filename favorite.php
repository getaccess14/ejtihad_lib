...
<body>
  ...
  <div class="right-column">
	...
	$search = "";
	$sql = "SELECT * FROM fav WHERE loggedUser = ?";
	$params = [$loggedUser];

	if (isset($_GET['search'])) {
	  $search = $_GET['search'];
	  $sql = "SELECT * FROM fav WHERE (title LIKE ? OR author LIKE ? OR major LIKE ? OR description LIKE ? OR item_id LIKE ?) AND loggedUser = ?";
	  $params = ["%$search%", "%$search%", "%$search%", "%$search%", "%$search%", $loggedUser];
	}
...
//gggggggggggggggggggggggggggg
	echo "Logged User: " . $loggedUser . "<br>";
	echo "Logged User: " . $search . "<br>";
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
		...
		echo '<button class="favorites-button" onclick="showPopup(\'' . $title . '\', \'' . $item_id . '\', \'' . $loggedUser . '\')">Unfavorite</button>';
  ...
  <script>
	...
	function RemoveFavorites() {
	  ...
	  var xhr = new XMLHttpRequest();
	  xhr.open("POST", "removeFavorite.php", true);
...
<div id="popup" class="popup">
  <div class="popup-content">
    ...
    <p class = "pop-txt" >Remove '<span id="fileTitle"></span>' from Your Favorites?</p>
    <button class = "add-button" onclick="RemoveFavorites()">Remove</button>
